 ...
 * file-system, this wont work out-of-the-box if the file is inside a jar. Also,
 * the url would differ in RAP.
 * 
 ...
 * 
 * Note that loading the html from another server wont work in RAP due to the
...
final class HtmlLoader {
    ...
    private static String getHtmlContent(final String url) {
        final StringBuffer html = getFileContent(url);
        inlineScripts(html);
    ...
    private static StringBuffer getFileContent(final String file) {
        ...
        final InputStream stream = HtmlLoader.class.getClassLoader().getResourceAsStream(file);
        if (stream != null) {
    ...
    private static void inlineScripts(final StringBuffer html) {
        final String srcAttrStr = "src=\"./";
        final String quotStr = "\"";
        final String tagStr = "<script ";
        final String closingTagStr = "</script>";
        final String newTagStr = "<script type=\"text/javascript\">";
        int offset = html.length();
        while ((offset = html.lastIndexOf(tagStr, offset)) != -1) {
            final int closeTag = html.indexOf(closingTagStr, offset);
            final int srcAttr = html.indexOf(srcAttrStr, offset);
            if (srcAttr != -1 && srcAttr < closeTag) {
                final int srcAttrStart = srcAttr + srcAttrStr.length();
                final int srcAttrEnd = html.indexOf(quotStr, srcAttrStart);
                if (srcAttrEnd != -1) {
                    final String filename = html.substring(srcAttrStart, srcAttrEnd);
                    final StringBuffer newScriptTag = new StringBuffer();
