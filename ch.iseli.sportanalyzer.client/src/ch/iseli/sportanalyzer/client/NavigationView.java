...
public class NavigationView extends ViewPart {
	...
	class ViewLabelProvider extends LabelProvider {
		...
		public String getText(Object obj) {
			TrainingCenterDatabaseT t = (TrainingCenterDatabaseT) obj;
			return t.getActivities().getActivity().get(0).getId().toString();
		}
	...
	public void createPartControl(Composite parent) {
		...
		viewer.addDoubleClickListener(new IDoubleClickListener() {
			...
			public void doubleClick(DoubleClickEvent event) {
				IStructuredSelection selection = (IStructuredSelection) event
						.getSelection();
				TrainingCenterDatabaseT firstElement = (TrainingCenterDatabaseT) selection
						.getFirstElement();
				TrainingCenterDataCache.setSelectedRun(firstElement);
				...
				try {
					PlatformUI
							...
							.getActivePage()
							.showView(
									SingleActivityViewPart.ID,
									TrainingCenterDataCache.getSelected()
											.toString(), 1);
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				TrainingCenterDatabaseT firstElement = (TrainingCenterDatabaseT) selection
						.getFirstElement();
				writeToStatusLine(firstElement);
			}
			...
			private void writeToStatusLine(TrainingCenterDatabaseT selectedRun) {
				getViewSite()
						...
						.setMessage(
								"Run vom "
										+ selectedRun.getActivities()
												.getActivity().get(0).getId()
												.toString());
			}
		});
						...
						Display.getDefault().asyncExec(new Runnable() {
							...
							public void run() {
								try {
									...
									TrainingCenterDataCache.initCache(allRuns);
								} catch (Exception e) {
									e.printStackTrace();
								}
							}
	...
	private IConvert2Tcx getConverterImplementation(
			IConfigurationElement[] configurationElementsFor) {
		for (final IConfigurationElement element : configurationElementsFor) {
			try {
				return (IConvert2Tcx) element
