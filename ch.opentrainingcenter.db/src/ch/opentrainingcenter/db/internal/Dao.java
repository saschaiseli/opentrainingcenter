		...
		if (session == null) {
			...
			case PRODUCTION:
				sessionFactory = configuration
						.configure("hibernate.cfg.xml").buildSessionFactory(); //$NON-NLS-1$
				break;
			case DEVELOPING:
				sessionFactory = configuration.configure(
						"hibernate_dev.cfg.xml").buildSessionFactory(); //$NON-NLS-1$
				break;
			default:
				sessionFactory = configuration.configure(
						"hibernate_junit.cfg.xml").buildSessionFactory(); //$NON-NLS-1$
			}
