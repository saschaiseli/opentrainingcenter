        CREATE TABLE PUBLIC.ATHLETE (ID INTEGER NOT NULL,NAME VARCHAR(255), AGE INTEGER NOT NULL, MAXHEARTRATE INTEGER, PRIMARY KEY (ID))
        CREATE TABLE PUBLIC.HEALTH ( ID INTEGER NOT NULL, WEIGHT DOUBLE, CARDIO INTEGER, ID_FK_ATHLETE INTEGER,DATEOFMEASURE DATE, PRIMARY KEY (ID))
        CREATE TABLE PUBLIC.IMPORTED (ID INTEGER NOT NULL, ID_FK_ATHLETE INTEGER,IMPORTED_DATE TIMESTAMP, ACTIVITY_ID TIMESTAMP,COMMENTS VARCHAR(2147483647),ID_FK_TRAININGTYPE INTEGER,ID_FK_TRAINING INTEGER, PRIMARY KEY (ID))
        CREATE TABLE PUBLIC.WEATHER (ID INTEGER NOT NULL, WETTER VARCHAR(2147483647), IMAGEICON VARCHAR(2147483647), PRIMARY KEY (ID))
        CREATE TABLE PUBLIC.TRAININGTYPE (ID INTEGER NOT NULL, TITLE VARCHAR(2147483647), DESCRIPTION VARCHAR(2147483647), IMAGEICON VARCHAR(2147483647), PRIMARY KEY (ID))
        CREATE TABLE PUBLIC.TRAINING (ID INTEGER NOT NULL, DATUM VARCHAR(2147483647), DAUER DOUBLE, LAENGEINMETER DOUBLE, AVERAGEHEARTBEAT INTEGER,MAXHEARTBEAT INTEGER, MAXSPEED DOUBLE, NOTE VARCHAR(2147483647),ID_FK_WEATHER INTEGER, PRIMARY KEY (ID))
        CREATE TABLE PUBLIC.PLANUNGWOCHE ( ID INTEGER NOT NULL, KW INTEGER, JAHR INTEGER,KMPROWOCHE INTEGER, INTERVAL BOOLEAN,ID_FK_ATHLETE INTEGER, PRIMARY KEY (ID))

        ALTER TABLE PUBLIC.HEALTH ADD FOREIGN KEY (ID_FK_ATHLETE) REFERENCES OTC.PUBLIC.ATHLETE (ID)
        ALTER TABLE PUBLIC.IMPORTED ADD FOREIGN KEY (ID_FK_ATHLETE) REFERENCES OTC.PUBLIC.ATHLETE (ID)
        ALTER TABLE PUBLIC.TRAINING ADD FOREIGN KEY (ID_FK_WEATHER) REFERENCES OTC.PUBLIC.WEATHER (ID)
        ALTER TABLE PUBLIC.IMPORTED ADD FOREIGN KEY (ID_FK_TRAININGTYPE) REFERENCES OTC.PUBLIC.TRAININGTYPE (ID)
        ALTER TABLE PUBLIC.PLANUNGWOCHE ADD FOREIGN KEY (ID_FK_ATHLETE) REFERENCES OTC.PUBLIC.ATHLETE (ID)

        CREATE sequence IMPORT_ID_SEQUENCE;
        CREATE sequence ATHLETE_ID_SEQUENCE;
        CREATE sequence TRAINING_ID_SEQUENCE;
        CREATE sequence HEALTH_ID_SEQUENCE;
        CREATE sequence PLANUNG_ID_SEQUENCE;

        INSERT INTO PUBLIC.ATHLETE (ID,NAME,AGE,MAXHEARTRATE) values(0,'Sascha',37, 200);
