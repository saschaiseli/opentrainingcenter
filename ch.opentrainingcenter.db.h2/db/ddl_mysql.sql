        CREATE TABLE ATHLETE (ID INTEGER NOT NULL,NAME VARCHAR(255), AGE INTEGER NOT NULL, MAXHEARTRATE INTEGER, PRIMARY KEY (ID))
        CREATE TABLE HEALTH ( ID INTEGER NOT NULL, WEIGHT DOUBLE, CARDIO INTEGER, ID_FK_ATHLETE INTEGER,DATEOFMEASURE DATE, PRIMARY KEY (ID))
        CREATE TABLE IMPORTED (ID INTEGER NOT NULL, ID_FK_ATHLETE INTEGER,IMPORTED_DATE TIMESTAMP, ACTIVITY_ID TIMESTAMP,COMMENTS VARCHAR(2147483647),ID_FK_TRAININGTYPE INTEGER,ID_FK_TRAINING INTEGER, PRIMARY KEY (ID))
        CREATE TABLE WEATHER (ID INTEGER NOT NULL, WETTER VARCHAR(2147483647), IMAGEICON VARCHAR(2147483647), PRIMARY KEY (ID))
        CREATE TABLE TRAININGTYPE (ID INTEGER NOT NULL, TITLE VARCHAR(2147483647), DESCRIPTION VARCHAR(2147483647), IMAGEICON VARCHAR(2147483647), PRIMARY KEY (ID))
        CREATE TABLE TRAINING (ID INTEGER NOT NULL, DATUM VARCHAR(2147483647), DAUER DOUBLE, LAENGEINMETER DOUBLE, AVERAGEHEARTBEAT INTEGER,MAXHEARTBEAT INTEGER, MAXSPEED DOUBLE, NOTE VARCHAR(2147483647),ID_FK_WEATHER INTEGER, PRIMARY KEY (ID))
        CREATE TABLE PLANUNGWOCHE ( ID INTEGER NOT NULL, KW INTEGER, JAHR INTEGER,KMPROWOCHE INTEGER, INTERVAL BOOLEAN,ID_FK_ATHLETE INTEGER, PRIMARY KEY (ID))

        ALTER TABLE HEALTH ADD FOREIGN KEY (ID_FK_ATHLETE) REFERENCES OTC.ATHLETE (ID)
        ALTER TABLE IMPORTED ADD FOREIGN KEY (ID_FK_ATHLETE) REFERENCES OTC.ATHLETE (ID)
        ALTER TABLE TRAINING ADD FOREIGN KEY (ID_FK_WEATHER) REFERENCES OTC.WEATHER (ID)
        ALTER TABLE IMPORTED ADD FOREIGN KEY (ID_FK_TRAININGTYPE) REFERENCES OTC.TRAININGTYPE (ID)
        ALTER TABLE PLANUNGWOCHE ADD FOREIGN KEY (ID_FK_ATHLETE) REFERENCES OTC.ATHLETE (ID)
        ...
        INSERT INTO ATHLETE (ID,NAME,AGE,MAXHEARTRATE) values(0,'Sascha',37, 200);
